...
流程：原图 -> 本地定位/裁剪完整车牌 -> SWE 默认 Agent 流程 -> JSON 结果。

用法：
    python -m license.v2.run_forgery data/raw/images/example.jpg
    python -m license.v2.run_forgery image.jpg --plate-box 120,340,680,160
"""
...
SWE_TRAJECTORY_ROOT = PROJECT_ROOT / "runtime" / "license-v2" / "trajectories" / "run"
CODE_VERSION = "v2.17.0"
SWE_MAX_ITERATIONS = 5
...
def find_plate_box(image: Image.Image, max_side: int = 1920) -> Optional[Tuple[int, int, int, int]]:
    """v1 风格的本地车牌定位：HSV 掩码、闭/开运算、连通轮廓与形状筛选。"""
    ...
    for start in range(work_w * work_h):
        ...
        if area < 250 or not 1.7 <= ratio <= 8.5 or coverage < 0.18:
            continue
        # 车牌通常接近 3:1，面积和色彩覆盖率越高越好。
        ratio_score = math.exp(-abs(math.log(ratio / 3.1)))
        score = count * coverage * ratio_score
        candidates.append((score, min_x, min_y, max_x, max_y))

    if not candidates:
        return None
    _, min_x, min_y, max_x, max_y = max(candidates)
    # 与 v1 一致，对检测框四周各增加 10% 边距。
    padding_x = max(2, round((max_x - min_x + 1) * 0.10))
    padding_y = max(2, round((max_y - min_y + 1) * 0.10))
    left = max(0, min_x - padding_x)
    top = max(0, min_y - padding_y)
    right = min(work_w, max_x + padding_x + 1)
    bottom = min(work_h, max_y + padding_y + 1)
    return (
        round(left / scale),
        round(top / scale),
...
async def process_image_async(
    ...
    if not use_plate_crop:
        ...
        box = find_plate_box(image)
        if box is None:
            raise ValueError("本地 HSV 掩码未定位到蓝/绿车牌；请用 --plate-box x,y,width,height 指定车牌框")
        localization = {"source": "local_hsv_contour"}
    if use_plate_crop:
    ...
    prompt = build_prompt(plate.width, plate.height, input_is_plate_crop=use_plate_crop)
    own_agent = agent is None
    ...
    data_scope = {
        ...
        "model_input": "one complete cropped plate image" if use_plate_crop else "one uncropped original image",
        ...
        "original_image_sent_to_model_for_localization": not use_plate_crop,
        "original_image_sent_to_model_for_judgment": not use_plate_crop,
    }
    if batch_scope:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="v2 车牌伪造筛查（SWE 默认 Agent 流程）")
    ...
    parser.add_argument("--output-dir", default="runtime/license-v2/single/results", help="裁剪图和结果保存目录")
    parser.add_argument("--answers-dir", default="runtime/license-v2/single/answers", help="最终答案保存目录")
