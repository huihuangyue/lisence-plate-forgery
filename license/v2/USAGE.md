...
```bash
python -m license.v2.run_forgery data/raw/images/AADY1439.JPG
```

运行流程为：本地 HSV 掩码定位并裁剪完整车牌，再交给 SWE Agent 处理。初始模型请求只含该裁剪图；SWE 保留默认工具、关闭扩展推理，单图最多运行 5 轮，因此模型若自行选择执行代码，会产生额外轮次及工具上下文。判别提示词先要求检查裁剪是否完整；只有完整时才判断每个字符。

裁剪完成后，程序会将该图实际的宽 × 高（像素）动态加入系统提示词。模型返回的 `bbox` 必须以这个尺寸的裁剪图左上角为原点。

单图最多 5 次模型回复：前 4 次仍可调用 `execute_code`，最后一次由程序移除该工具并强制调用 `finish`，以返回最终 JSON。若首次输出不合格，重试只能使用本图剩余的调用预算，绝不会超过 5 次。
...
```bash
python -m license.v2.run_forgery data/raw/images/AADY1439.JPG --no-crop-plate
```
...
```bash
python -m license.v2.run_batch --limit 20 --workers 4
```
